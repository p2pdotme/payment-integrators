...
describe("PayQRVault — custody, airtight pull, lock, migration, multi-owner", function () {
  ...
  it("AIRTIGHT: only the linked integrator can pull", async function () {
    ...
    await expect(vault.connect(owner).pull(owner.address, USDC(1))).to.be.revertedWithCustomError(
      vault,
      "NotIntegrator"
    );
    await expect(
      vault.connect(attacker).pull(attacker.address, USDC(1))
    ...
    await expect(integrator2.doPull(alice.address, USDC(1))).to.be.revertedWithCustomError(
      vault,
      "NotIntegrator"
    );
  });
  ...
  it("AIRTIGHT HANDSHAKE: setIntegrator rejects an integrator that doesn't point back", async function () {
    ...
    await expect(
      vault.connect(owner).setIntegrator(await stray.getAddress())
  ...
  it("pull rejects zero address / zero amount", async function () {
    await expect(integrator.doPull(ethers.ZeroAddress, USDC(1))).to.be.revertedWithCustomError(
      vault,
      "BadPull"
    );
    await expect(integrator.doPull(alice.address, 0)).to.be.revertedWithCustomError(
      vault,
      "BadPull"
    );
  });
  ...
  it("KILL-SWITCH: lock() blocks ALL pulls until unlock()", async function () {
    ...
    await expect(integrator.doPull(alice.address, USDC(1))).to.be.revertedWithCustomError(
      vault,
      "VaultLocked"
    );
  ...
  it("kill-switch lock/unlock is owner-gated; setIntegrator is super-admin-gated", async function () {
    ...
    await expect(
      vault.connect(alice).setIntegrator(await integrator2.getAddress())
    ).to.be.revertedWithCustomError(vault, "OnlySuperAdmin");
    await expect(
      vault.connect(attacker).setIntegrator(await integrator2.getAddress())
    ).to.be.revertedWithCustomError(vault, "OnlySuperAdmin");
  ...
  it("SUPER-ADMIN owner set: only super-admin adds/removes owners; owners get the kill-switch, not governance", async function () {
    ...
    await expect(vault.connect(owner).addOwner(alice.address))
      .to.emit(vault, "OwnerAdded")
    ...
    await expect(vault.connect(alice).addOwner(attacker.address)).to.be.revertedWithCustomError(
      vault,
      "OnlySuperAdmin"
    );
    await expect(vault.connect(alice).removeOwner(owner.address)).to.be.revertedWithCustomError(
      vault,
      "OnlySuperAdmin"
    );
    // A genuine non-owner can't either.
    await expect(vault.connect(stranger).removeOwner(owner.address)).to.be.revertedWithCustomError(
      vault,
      "OnlySuperAdmin"
    );
    // The super-admin can never be removed as an owner (even by itself).
    await expect(vault.connect(owner).removeOwner(owner.address)).to.be.revertedWithCustomError(
      vault,
  ...
  it("SUPER-ADMIN handoff: TWO-STEP (propose + accept), super-admin only, keeps root an owner", async function () {
    ...
    await expect(
      vault.connect(attacker).transferSuperAdmin(attacker.address)
    ).to.be.revertedWithCustomError(vault, "OnlySuperAdmin");
    await expect(
      vault.connect(owner).transferSuperAdmin(owner.address)
    ...
    await expect(vault.connect(owner).transferSuperAdmin(alice.address))
      .to.emit(vault, "SuperAdminTransferStarted")
    ...
    await expect(vault.connect(attacker).acceptSuperAdmin()).to.be.revertedWithCustomError(
      vault,
      "OnlySuperAdmin"
    );
    ...
    await expect(vault.connect(alice).acceptSuperAdmin())
      .to.emit(vault, "SuperAdminTransferred")
      .withArgs(owner.address, alice.address);
    ...
    await expect(vault.connect(owner).addOwner(attacker.address)).to.be.revertedWithCustomError(
      vault,
      "OnlySuperAdmin"
    );

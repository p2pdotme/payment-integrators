...
/**
 * Deploy LotPotCheckoutIntegratorV2 and UserProxyV2.
 *
 * V2 changes from V1:
 *   - Deploy UserProxyV2 (explicit impl arg) instead of UserProxy
 *   - Deploy LotPotCheckoutIntegratorV2 (explicit _owner and _proxyImpl constructor args)
 *   - Constructor signature: (diamond, usdc, owner, proxyImpl, megapot, batchFacilitator,
 *     jackpotNft, baseTxLimit, dailyTxCountLimit, source)
 *   - V2 adds: deprecation flag, activity clock on proxies, sweepStale() for ops recovery
 *
 ...
 *
 * Output:
 *   Prints LotPotCheckoutIntegratorV2 address and UserProxyV2 impl address.
 *   These are needed for the Diamond's `setLotpotBuyerCashback(200, v2IntegratorAddr)`
 *   admin call (run separately, outside this script).
 */
...
async function main() {
  ...
  // Deploy UserProxyV2 implementation
  console.log("Deploying UserProxyV2 implementation...");
  const UserProxyV2Factory = await ethers.getContractFactory("UserProxyV2");
  const userProxyV2Impl = await UserProxyV2Factory.deploy();
  const userProxyV2ImplTx = userProxyV2Impl.deploymentTransaction();
  await userProxyV2ImplTx?.wait(5);
  const userProxyV2ImplAddress = await userProxyV2Impl.getAddress();
  console.log(`UserProxyV2 implementation deployed to: ${userProxyV2ImplAddress}`);
  console.log("");

  // Deploy LotPotCheckoutIntegratorV2
  console.log("Deploying LotPotCheckoutIntegratorV2...");
  const IntegratorV2 = await ethers.getContractFactory("LotPotCheckoutIntegratorV2");
  const integratorV2 = await IntegratorV2.deploy(
    DIAMOND_ADDRESS,
    USDC_ADDRESS,
    await deployer.getAddress(), // _owner
    userProxyV2ImplAddress, // _proxyImpl
    MEGAPOT_ADDRESS,
    BATCH_FACILITATOR_ADDRESS,
    JACKPOT_NFT_ADDRESS,
  ...
  const deployTx = integratorV2.deploymentTransaction();
  await deployTx?.wait(5);

  const integratorV2Address = await integratorV2.getAddress();
  const proxyImpl = await integratorV2.proxyImpl();
  console.log(`LotPotCheckoutIntegratorV2 deployed to: ${integratorV2Address}`);
  console.log(`UserProxyV2 implementation:             ${proxyImpl}`);

  const code = await ethers.provider.getCode(integratorV2Address);
  if (code === "0x" || code.length <= 2)
    throw new Error(`Contract has no code at ${integratorV2Address}`);

  console.log("");
  console.log("=== Deployment Summary ===");
  console.log(`LotPotIntegratorV2:    ${integratorV2Address}`);
  console.log(`UserProxyV2 impl:      ${proxyImpl}`);
  console.log(`Diamond:               ${await integratorV2.diamond()}`);
  console.log(`USDC:                  ${await integratorV2.usdc()}`);
  console.log(`Megapot:               ${await integratorV2.megapot()}`);
  console.log(`JackpotNFT:            ${await integratorV2.jackpotNft()}`);
  console.log(`Owner:                 ${await integratorV2.owner()}`);
  console.log(
    `Base TX Limit:         ${ethers.formatUnits(await integratorV2.baseTxLimit(), 6)} USDC`
  );
  console.log(
    `Daily TX Count:        ${(await integratorV2.dailyTxCountLimit()).toString()} per day`
  );
  console.log("");
  console.log("=== V2-Specific Addresses (for Diamond admin setup) ===");
  console.log(`V2 Integrator Address: ${integratorV2Address}`);
  console.log(`V2 Proxy Impl Address: ${proxyImpl}`);
  console.log("");
  console.log("Next steps:");
  console.log(`  1. Register on Diamond (super-admin):`);
  console.log(`       registerIntegrator(${integratorV2Address}, false, ${proxyImpl})`);
  console.log(
  ...
  console.log(`  2. Set lottery buyer cashback (super-admin):`);
  console.log(`       setLotpotBuyerCashback(200, ${integratorV2Address})`);
  console.log(`  3. Set currency rates:   setRpToUsdc(currency, rate)`);
  console.log(
    `  4. Coordinate with Megapot's owner to allowlist this integrator on BatchPurchaseFacilitator.`
  );

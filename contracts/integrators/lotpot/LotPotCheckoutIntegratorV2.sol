...
/**
 * @title LotPotCheckoutIntegratorV2
 ...
 *
 *         Placement and USDC custody flow through a per-user UserProxyV2
 ...
 *         exit as Megapot tickets — UserProxyV2 disables both user-initiated
 ...
 *         - `deprecated` public flag + `deprecate()` onlyOwner + `Deprecated` event
 *         - `_ensureProxy` calls `UserProxyV2.initialize()` post-clone to anchor
 *           the 90-day sweep clock
 *         - `adminEnsureProxy(user)` onlyOwner to materialize a proxy without
 *           placing an order (starts the sweep clock on never-engaged users
 *           who have accumulated cashback at a pre-computed CREATE2 address)
 */
contract LotPotCheckoutIntegratorV2 is IP2PIntegrator {
    ...
    /// @notice When true, the integrator is deprecated. UserProxyV2.sweepStale()
    ///         treats this flag as an immediate unlock so ops can recover
    ///         stranded USDC from proxies without waiting 90 days.
    bool public deprecated;

    /// @dev Emitted once when the owner marks the integrator deprecated.
    event Deprecated();
    ...
    /// @notice Marks this integrator as deprecated. UserProxyV2.sweepStale()
    ///         will unlock immediately (no 90-day wait) for all proxies once
    ///         this flag is set, allowing ops to recover stranded cashback
    ///         USDC from never-engaged users.
    ...
    function _route(
        ...
        // Credit accounting: existing USDC on the user's proxy from a
        // previously skipped fulfillment counts as a balance against this
        // purchase. Skip the Diamond entirely if credit covers the total;
        // otherwise place a Diamond order for only the delta.
        ...
        uint256 delta = totalPrice - credit;
    ...
    function _redeemFromCredit(
        ...
        if (quantity <= MAX_DIRECT_TICKETS) {
            ...
            bytes memory result = UserProxyV2(payable(proxy)).execute(
                megapot,
                data,
                address(usdc),
                totalPrice
            ...
            UserProxyV2(payable(proxy)).transferERC20ToIntegrator(address(usdc), totalPrice);
    ...
    function _placeOrder(
        ...
        bytes memory result = UserProxyV2(payable(proxy)).execute(diamond, data, address(usdc), 0);
    ...
    function _fulfillDirect(
        ...
        bool execOk;
        try
            UserProxyV2(payable(proxy)).execute(megapot, data, address(usdc), expectedTotal)
    ...
    function _fulfillBatch(
        ...
        // external call.
        UserProxyV2(payable(proxy)).transferERC20ToIntegrator(address(usdc), expectedTotal);
    ...
    function _ensureProxy(address user) internal returns (address proxy) {
        ...
        if (proxy.code.length == 0) {
            ...
            UserProxyV2(payable(deployed)).initialize();
            emit UserProxyDeployed(user, proxy);
        }
    }

    /// @notice Materializes a user's proxy without placing an order. Used by
